...
def parse_arguments():
    ...
    #     TransELS(args.gtf_annotation_path,args.short_read_sam_path,args.long_read_sam_path,args.output_path,alpha,beta,1e-6,args.filtering,args.multi_mapping_filtering,args.SR_quantification_option,SR_fastq_list,args.reference_genome,args.training,args.DL_model,args.assign_unique_mapping_option,args.threads,READ_JUNC_MIN_MAP_LEN=args.READ_JUNC_MIN_MAP_LEN)
        ...
        is_multi = (lr_sam_list is not None and len(lr_sam_list) > 1) or \
                   (sr_sam_list is not None and len(sr_sam_list) > 1)
        if is_multi:
            # 多平台模式
            ...
            EM_hybrid_multi(args.gtf_annotation_path,sr_sam_list or [],lr_sam_list or [],args.output_path,multi_mapping_filtering=args.multi_mapping_filtering,threads=args.threads,READ_JUNC_MIN_MAP_LEN=args.READ_JUNC_MIN_MAP_LEN,EM_choice=em_choice,lr_weights=_lr_weights,sr_weights=_sr_weights)
            return
