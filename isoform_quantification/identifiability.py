...
2. Restricted Fisher information (multinomial model):
     F_L = J_L^T W_L J_L,   W_L = diag(1 / p^(L)),  p^(L) = A^(L) theta
     F_S = J_S^T W_S J_S,   W_S = diag(1 / p^(S)),  p^(S) = A^(S) phi(theta)
     F_H = F_L + F_S
...
def _compute_se_ci(F_tan, B, theta_hat, gamma=0.05):
    ...
    An isoform t is marked null (SE = nan) only when its tangent-space representation
    b_t = B^T e_t lies predominantly (> 50%) in the null space of F_tan.
    When F_tan is positive definite (fisher_identifiable=1), no null space exists and
    ...
    if np.any(null_mask):
        ...
        is_null = null_proj / np.maximum(b_norm_sq, 1e-30) > 0.5
    else:
    ...
    if has_sr:
        ...
        if len(sr_ref_mds) > 1:
            for k, (sr_md, J_Sk, F_Sk) in enumerate(zip(sr_ref_mds, J_S_list, F_S_list)):
                ...
                fc_k, fsmin_k, fi_k = _metrics_from_Fisher(F_Sk)
                result[f'SR_{k+1}'] = {
                    'k_orig': k_orig_k,
                    'k_value': kv_k, 'sigma_max': sm_k, 'sigma_min': smi_k,
...
def compute_and_output_identifiability(output_path, sr_matrix_input, lr_matrix_input, sr_theoretical_input=None):
    """
    ...
    If expression_isoform.out is found in output_path, EM theta is used as the
    operating point for restricted Jacobian k-values; otherwise only k_orig is output.
    ...
    rows = []
    se_ci_rows = []
    for chr_name, gene_name in sorted(all_gene_keys):
        ...
        # isoform_names_indics：优先从第一个 LR 样本取，无 LR 则取第一个 SR 样本
        ...
        # Per-sample read counts（平台存在但计数为 0 时用 1 兜底）
        ...
        # 样本数与 N 列表对齐（取较短者，多余 N 截断，不足补 1）
        ...
        if has_em and theta_hat_em is not None:
            ...
            for tag_k in lr_per_sample_keys:
                ...
                row.update({
                    ...
                    f'{tag_k}_rfisher_k_value':    _fmt(m_k.get('k_value', math.nan)),
            ...
            if has_LR and not lr_platform_redundant:
                ...
                row.update({
                    ...
                    'LR_rfisher_k_value':    _fmt(lr_m.get('k_value', math.nan)),
            ...
            if has_SR and not sr_platform_redundant:
                ...
                row.update({
                    ...
                    'SR_rfisher_k_value':    _fmt(sr_m.get('k_value', math.nan)),
            ...
            if has_Hybrid:
                ...
                row.update({
                    ...
                    'Hybrid_rfisher_k_value': _fmt(h.get('k_value', math.nan)),
        ...
        # ---- 收集 per-isoform SE / CI 行（只含实际存在平台的列） ----
        # 有 EM 文件时始终输出所有基因的行；theta_hat_em 为 None 时输出全 null
        if has_em:
            iso_indics = ref_md.get('isoform_names_indics', {})
            idx_to_name = {v: k for k, v in iso_indics.items()}
            if theta_hat_em is not None:
                # 顺序：分样本 → 非冗余平台合并 → Hybrid（最后）
                active_tags = (
                ...
                for t in range(T):
                    ...
                    se_ci_rows.append(iso_row)
            else:
                # EM 中找不到该基因（TPM 全 0 或未量化），输出全 null 行
                for t in range(T):
                    iso_name = idx_to_name.get(t, f'isoform_{t}')
                    se_ci_rows.append({
                        'gene':      gene_name,
