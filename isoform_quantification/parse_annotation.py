...
def parse_annotation(ref_annotation_path,threads):
    ...
    num_exons = 0
    # 逐行解析 GTF文件，构建基因-外显子区间的字典
    for line in file_read:
        if line.lstrip()[0] == "#":  # 跳过注释行
            continue
        fields = line.split('\t')
        if (fields[2] != 'exon'):
            continue          # 只处理 exon 行，跳过 gene/transcript 行
        chr_name = fields[0]
        converted_chr_name = sync_reference_name(fields[0])
        if (converted_chr_name.isnumeric()):
            chr_name = converted_chr_name       # 统一染色体名：chr1 → 1
        # if chr_name == '':
        #     continue
        num_exons += 1
        ...
        # 记录该 isoform 的原始外显子坐标，后续会根据基因的 exon 划分结果进行转换
    ...
    for chr_name in raw_isoform_exons_dict:
        for gene_name in raw_isoform_exons_dict[chr_name]:
            for isoform_name in raw_isoform_exons_dict[chr_name][gene_name].copy():
                # 对每个 isoform 的外显子按起始位置排序，拆成 start_pos 和 end_pos 两个列表
    ...
    # 将重叠外显子拆分为不重叠的小区间，并排序，然后为每个基因的这些小区间的起始和结束位置建立一个索引，记录在 gene_points_dict 中，索引的值为该位置在基因的所有小区间中按升序排序后的顺序
            ...
            for [start_pos,end_pos,_] in gene_exons_dict[chr_name][gene_name]:
                for pos in [start_pos,end_pos]:
                    if pos not in gene_points_dict[chr_name][gene_name]:
                        ...
                        point_index += 1
    # 根据基因的 exon 划分结果和 isoform 的原始外显子坐标，生成每个 isoform 的 exon/junction 区间，并记录在 gene_regions_dict 中，同时计算每个 isoform 的 exon/junction 区间总长度，记录在 isoforms_regions_len_dict 中                    
