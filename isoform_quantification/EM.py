...
from EM_hybrid.EM_hybrid_community import EM_algo_hybrid,EM_algo_hybrid_multi
from EM_hybrid.EM_LR_alone_community import EM_algo_LR_alone,EM_algo_LR_alone_multi
...
def map_long_reads(long_read_alignment_file_path,READ_JUNC_MIN_MAP_LEN,CHR_LIST,output_path,threads,multi_mapping_filtering,lr_align_dir=None):
    ...
    parse_alignment_EM(long_read_alignment_file_path,READ_JUNC_MIN_MAP_LEN,output_path,threads,CHR_LIST,lr_align_dir=lr_align_dir)
    ...
    long_read_gene_regions_read_count,long_read_gene_regions_read_length,total_long_read_length,num_LRs = get_region_read_count_length(LR_gene_regions_dict,output_path,lr_align_dir=lr_align_dir)
    long_read_gene_matrix_dict = generate_all_feature_matrix_long_read(gene_isoforms_dict,LR_gene_regions_dict,long_read_gene_regions_read_count,long_read_gene_regions_read_length,LR_genes_regions_len_dict,raw_isoform_exons_dict)
    LR_kvalue_dict = {}
...
def EM_hybrid_multi(ref_file_path,sr_sam_list,lr_sam_list,output_path,multi_mapping_filtering='best',threads=1,READ_JUNC_MIN_MAP_LEN=15,EM_choice='LIQA_modified',lr_weights=None,sr_weights=None,use_quality_weights=False,alpha='adaptive'):
    """多平台EM：支持多个LR SAM和/或多个SR SAM，等权重(w_p=1)合并。"""
    try:
        shutil.rmtree(output_path)
    except:
        pass
    ...
    # 注释解析：使用第一个文件代表做GTF解析
    first_lr = lr_sam_list[0] if lr_sam_list else None
    first_sr = sr_sam_list[0] if sr_sam_list else None
    print('[INFO] Preprocessing...',flush=True)
    CHR_LIST = parse_and_dump_dict(ref_file_path,first_sr,first_lr,output_path,multi_mapping_filtering,threads,READ_JUNC_MIN_MAP_LEN=READ_JUNC_MIN_MAP_LEN)

    # 映射每个LR数据集到独立目录
    lr_align_dirs = []
    for i,lr_sam in enumerate(lr_sam_list):
        lr_align_dir = f'{output_path}/temp/LR_alignments_lr{i}'
        file_stats = os.stat(lr_sam)
        t = threads if file_stats.st_size/1024/1024 >= 100 else 1
        map_long_reads(lr_sam,READ_JUNC_MIN_MAP_LEN,CHR_LIST,output_path,t,multi_mapping_filtering,lr_align_dir=lr_align_dir)
        lr_align_dirs.append(lr_align_dir)
    ...
    print('[INFO] Start quantification...',flush=True)
    if sr_sam_list:
        EM_algo_hybrid_multi(isoform_len_dict,isoform_gene_dict,gene_isoforms_dict,sr_sam_list,lr_align_dirs,output_path,threads,EM_choice,lr_weights=lr_weights,sr_weights=sr_weights,use_quality_weights=use_quality_weights,alpha=alpha)
    else:
        EM_algo_LR_alone_multi(isoform_len_dict,isoform_gene_dict,lr_align_dirs,output_path,threads,EM_choice)
    try:
        Path(f'{output_path}/temp_lr.sam').unlink()
        Path(f'{output_path}/temp_lr.sorted.sam').unlink()
    except:
