...
def parse_alignment_iteration(alignment_file_path, READ_JUNC_MIN_MAP_LEN,map_f,CHR_LIST,output_path,lr_align_dir,aln_line_marker):
    os.nice(10)
    if lr_align_dir is None:
        lr_align_dir = f'{output_path}/temp/LR_alignments'
    Path(lr_align_dir).mkdir(exist_ok=True,parents=True)
        ...
        while True:
            ...
            try:
                ...
                if (mapping['read_mapped']):
                    ...
                    for mapping_area in mapping['mapping_area']: # 如果该read比对上了多个基因-区间，则把所有比对区域都考虑进来，之前是随机选择一个，现在改成把所有比对区域都考虑进来，增加统计的全面性
            ...
            if buffer_size > max_buffer_size:
                ...
                with open(f'{lr_align_dir}/reads_{worker_id}_{batch_id}','wb') as f:
                    pickle.dump([reads_isoform_info,read_len_dict],f)
                with open(f'{lr_align_dir}/dist_{worker_id}_{batch_id}','wb') as f:
                    pickle.dump([read_len_dist_dict,expression_dict],f)
                with open(f'{lr_align_dir}/read_count_length_{worker_id}_{batch_id}','wb') as f:
...
def get_aln_line_marker(alignment_file_path,threads):
    ...
    return aln_line_marker
def parse_alignment_EM(alignment_file_path,READ_JUNC_MIN_MAP_LEN,output_path,threads,CHR_LIST,lr_align_dir=None):
    if lr_align_dir is None:
        lr_align_dir = f'{output_path}/temp/LR_alignments'
    # print(threads)
    ...
    partial_read_alignment = partial(parse_alignment_iteration,alignment_file_path, READ_JUNC_MIN_MAP_LEN,map_f,CHR_LIST,output_path,lr_align_dir)
    futures = []
    ...
    pool.close()
    pool.join()
def get_region_read_count_length(gene_regions_dict,output_path,lr_align_dir=None):
    if lr_align_dir is None:
        lr_align_dir = f'{output_path}/temp/LR_alignments'
    gene_regions_read_count = {}
    gene_regions_read_length = {}
    for fpath in glob.glob(f'{lr_align_dir}/read_count_length_*_*'):
