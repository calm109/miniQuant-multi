...
def matrix_info_writer(args):
    # sr/lr matrix_input 可以是单个 dict 或 list of dict（多样本）
    sr_matrix_input, lr_matrix_input, gname, rname, gene_points_dict, output_path = args
    sr_matrix_list = [] if sr_matrix_input is None else (sr_matrix_input if isinstance(sr_matrix_input, list) else [sr_matrix_input])
    lr_matrix_list = [] if lr_matrix_input is None else (lr_matrix_input if isinstance(lr_matrix_input, list) else [lr_matrix_input])
    # 取第一个有数据的平台来读取 isoform 名称
    ref_matrix = (lr_matrix_list[0] if lr_matrix_list else sr_matrix_list[0] if sr_matrix_list else None)
    if ref_matrix is None or rname not in ref_matrix or gname not in ref_matrix[rname]:
        return
    bio = io.BytesIO()
    bio.write(str.encode('{}\n'.format(gname)))
    all_isoforms = ref_matrix[rname][gname]['isoform_names_indics'].keys()
    bio.write(str.encode('{}\n'.format(','.join(all_isoforms))))
    bio.write(str.encode('----------------------------\n'))
    for i, sr_dict in enumerate(sr_matrix_list):
        if rname not in sr_dict or gname not in sr_dict[rname]:
            continue
        label = 'SR_A' if len(sr_matrix_list) == 1 else f'SR_A_{i}'
        SR_all_coord_region = []
        for region, index in sr_dict[rname][gname]['region_names_indics'].items():
            SR_all_coord_region.append(get_coord(region, gene_points_dict, rname, gname))
        sr_A = sr_dict[rname][gname]['isoform_region_matrix']
        bio.write(str.encode(f'{label}\n'))
        np.savetxt(bio, sr_A, fmt='%1.3f', delimiter=',')
        if 'region_abund_matrix' in sr_dict[rname][gname]:
            sr_b = sr_dict[rname][gname]['region_abund_matrix']
            b_label = 'SR_b' if len(sr_matrix_list) == 1 else f'SR_b_{i}'
            bio.write(str.encode(f'{b_label}\n'))
            np.savetxt(bio, sr_b, fmt='%1.3f', delimiter=',')
            bio.write(str.encode('{}\n'.format(','.join(SR_all_coord_region))))
            bio.write(str.encode('{}\n'.format(sr_dict[rname][gname]['region_names_indics'])))
    for i, lr_dict in enumerate(lr_matrix_list):
        if rname not in lr_dict or gname not in lr_dict[rname]:
            continue
        lr_label = 'LR_A' if len(lr_matrix_list) == 1 else f'LR_A_{i}'
        LR_all_coord_region = []
        for region, index in lr_dict[rname][gname]['region_names_indics'].items():
            LR_all_coord_region.append(get_coord(region, gene_points_dict, rname, gname))
        lr_A = lr_dict[rname][gname]['isoform_region_matrix']
        bio.write(str.encode(f'{lr_label}\n'))
        np.savetxt(bio, lr_A, fmt='%1.3f', delimiter=',')
        if 'region_abund_matrix' in lr_dict[rname][gname]:
            lr_b = lr_dict[rname][gname]['region_abund_matrix']
            b_label = 'LR_b' if len(lr_matrix_list) == 1 else f'LR_b_{i}'
            bio.write(str.encode(f'{b_label}\n'))
            np.savetxt(bio, lr_b, fmt='%1.3f', delimiter=',')
            bio.write(str.encode('{}\n'.format(','.join(LR_all_coord_region))))
            bio.write(str.encode('{}\n'.format(lr_dict[rname][gname]['region_names_indics'])))
    out_str = bio.getvalue().decode('latin1')
    with open(output_path + f'/matrix_info/{gname}', 'w') as f:
        f.write(out_str)
def _extract_gene_matrix(matrix_input, gname, rname):
    """Extract single-gene slice to avoid pickling the full matrix dict per worker."""
    if matrix_input is None:
        return None
    if isinstance(matrix_input, list):
        result = []
        for m in matrix_input:
            if m is not None and rname in m and gname in m[rname]:
                result.append({rname: {gname: m[rname][gname]}})
        return result if result else None
    else:
        if rname in matrix_input and gname in matrix_input[rname]:
            return {rname: {gname: matrix_input[rname][gname]}}
        return None
def output_matrix_info(sr_matrix_input, lr_matrix_input, list_of_all_genes_chrs, gene_points_dict, output_path):
    Path(output_path+'/matrix_info/').mkdir(exist_ok=True,parents=True)
    list_of_args = []
    for gname, rname in list_of_all_genes_chrs:
        sr_gene = _extract_gene_matrix(sr_matrix_input, gname, rname)
        lr_gene = _extract_gene_matrix(lr_matrix_input, gname, rname)
        gp_gene = {rname: {gname: gene_points_dict[rname][gname]}} \
                  if rname in gene_points_dict and gname in gene_points_dict.get(rname, {}) else {}
        list_of_args.append((sr_gene, lr_gene, gname, rname, gp_gene, output_path))
    n_workers = min(config.threads, max(1, len(list_of_args)))
    chunksize, extra = divmod(len(list_of_args), n_workers)
    if extra:
        chunksize += 1
