...
import shutil
def TrEESR(ref_file_path,output_path,short_read_alignment_file_path,long_read_alignment_file_path,sr_region_selection,filtering,threads,lr_region_selection='read_length',READ_LEN=150,READ_JUNC_MIN_MAP_LEN=0):
    ...
    if isinstance(short_read_alignment_file_path, list):
        ...
        sr_sam_list = []
    sr_theoretical_matrix_dict = None  # 理论 A 矩阵（与原始 miniQuant 构建方式一致， real_length 模式），仅在 real_data 模式下额外计算
    if sr_sam_list:
        sr_matrix_list = []
    ...
    # SR 和 LR 均使用注释结构 + 读长过滤的理论矩阵（short_read_gene_matrix_dict，long_read_gene_matrix_dict），不依赖实际读段信息（保留了读段数为0的区域）
