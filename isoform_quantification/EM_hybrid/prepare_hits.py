...
def get_hits_dict(args):
    os.nice(10)
    worker_id,alignment_file_path,start_pos,end_pos,output_path,queue,isoform_index_dict,gene_isoform_index = args[:8]
    hits_dir = args[8] if len(args) > 8 else None
    if hits_dir is None:
        hits_dir = f'{output_path}/temp/hits_dict'
    frag_len_dict = {}
    read_index = 0
    ...
    theta_path = f'{hits_dir}/{worker_id}_theta_matrix.npz'
    np.savez_compressed(theta_path,theta=theta_matrix)
    frag_len_path = f'{hits_dir}/{worker_id}_frag_len_arr.npz'
    np.savez_compressed(frag_len_path,frag_len=np.hstack(all_frag_len_arr))
    frag_len_matrix_path = f'{hits_dir}/{worker_id}_frag_len.npz'
...
def get_aln_line_marker(alignment_file_path,threads):
    ...
    return byte_marker
def get_all_hits_dict(alignment_file_path,byte_marker,threads,output_path,isoform_index_dict,gene_isoform_index,hits_dir=None):
    if hits_dir is None:
        hits_dir = f'{output_path}/temp/hits_dict'
    ...
    for i in range(threads):
        ...
        args = i,alignment_file_path,start_pos,end_os,output_path,queue,isoform_index_dict,gene_isoform_index,hits_dir
...
def get_all_ant(args):
    worker_id,eff_len_arr,mean_f_len,std_f_len,output_path,num_batches = args[:6]
    hits_dir = args[6] if len(args) > 6 else None
    if hits_dir is None:
        hits_dir = f'{output_path}/temp/hits_dict'
    ...
    ANT_matrix.data[:] = 1/(std_f_len * np.sqrt(np.pi)) * np.e**(-1/2*((ANT_matrix.data-mean_f_len)/std_f_len)**2) # 注意！
    ANT_matrix = safe_divide_sparse(ANT_matrix,eff_len_arr)
    # try:
    #     Path(frag_len_matrix_path).unlink()
    # except:
    #     pass
    scipy.sparse.save_npz(f'{hits_dir}/{worker_id}_ANT.npz',ANT_matrix)
    # print(f'Get_all_ant: Worker {worker_id} done!')
    # print(datetime.datetime.now(),flush=True)
def get_ant_all_workers(eff_len_arr,mean_f_len,std_f_len,threads,output_path,num_batches_dict,isoform_index_dict,hits_dir=None):
    if hits_dir is None:
        hits_dir = f'{output_path}/temp/hits_dict'
    pool = mp.Pool(threads)
    futures = []
    for i in range(threads):
        args = i,eff_len_arr,mean_f_len,std_f_len,output_path,num_batches_dict[i],hits_dir
        futures.append(pool.apply_async(get_all_ant,(args,)))
    for future in futures:
        future.get()
    pool.close()
    pool.join()
def prepare_hits(SR_sam,output_path,isoform_index_dict,gene_isoform_index,threads,hits_dir=None):
    if hits_dir is None:
        hits_dir = f'{output_path}/temp/hits_dict'
    ...
    Path(hits_dir).mkdir(exist_ok=True,parents=True)
    # Path(f'{output_path}/temp/fragment_lengths/').mkdir(exist_ok=True,parents=True)
    theta_arr,mean_f_len,std_f_len,eff_len_arr,num_batches_dict = get_all_hits_dict(alignment_file_path,byte_marker,threads,output_path,isoform_index_dict,gene_isoform_index,hits_dir=hits_dir)
    # print('Get all hits dict done',flush=True)
    # print(datetime.datetime.now(),flush=True)
    get_ant_all_workers(eff_len_arr,mean_f_len,std_f_len,threads,output_path,num_batches_dict,isoform_index_dict,hits_dir=hits_dir)
