...
def EM_worker(worker_id,output_df,output_path,eff_len_arr,num_SRs,num_LRs):
    ...
    for community_idx, (community_isoform, community_id) in enumerate(zip(worker_isoform, worker_community)):
        ...
        if is_weighted:
            ...
            community_num_SRs_per_sample = [ant.shape[0] for ant in community_ANT_list]
            community_num_LRs_per_sample = [cp.shape[0] for cp in community_cond_prob_list]
        else:
            ...
            community_num_SRs_per_sample = [community_num_SRs]
            community_num_LRs_per_sample = [community_num_LRs]
        ...
        for i in range(num_iters):
            ...
            if diff[diff > min_diff].shape[0] == 0:
                break
        LR_expression = (community_num_LRs/num_LRs * theta_arr * 1e6) if num_LRs > 0 else np.zeros_like(theta_arr)
        ...
        for s_idx, n_s in enumerate(community_num_SRs_per_sample):
            SR_expression_df[f'community_num_SRs_{s_idx}'] = n_s
        for l_idx, n_l in enumerate(community_num_LRs_per_sample):
            SR_expression_df[f'community_num_LRs_{l_idx}'] = n_l
...
def EM_manager(isoform_gene_dict,isoform_index_dict,eff_len_arr,output_df,output_path,threads,num_SRs,num_LRs):
    ...
    # 动态列名：根据实际SR/LR样本数，每个样本单独一列
    n_sr_samples = len(config.sr_global_weights) if config.sr_global_weights is not None else (1 if num_SRs > 0 else 0)
    n_lr_samples = len(config.lr_global_weights) if config.lr_global_weights is not None else (1 if num_LRs > 0 else 0)
    ...
    # 只在有LR数据时写 LR_EM_expression.out 并合并 LR 列
    if num_LRs > 0:
    ...
    # 每个SR样本单独一列：num_expected_SRs_1, num_expected_SRs_2, ...
    sr_per_sample_cols = []
    for s_idx in range(n_sr_samples):
        col_name = f'num_expected_SRs_{s_idx+1}'
        all_SR_TPM_df[col_name] = all_SR_TPM_df[f'community_num_SRs_{s_idx}'] * all_SR_TPM_df['theta']
        sr_per_sample_cols.append(col_name)
    # 每个LR样本单独一列：num_expected_LRs_1, num_expected_LRs_2, ...（无LR时跳过）
    lr_per_sample_cols = []
    if num_LRs > 0:
        for l_idx in range(n_lr_samples):
            col_name = f'num_expected_LRs_{l_idx+1}'
            all_SR_TPM_df[col_name] = all_SR_TPM_df[f'community_num_LRs_{l_idx}'] * all_SR_TPM_df['theta']
            lr_per_sample_cols.append(col_name)
    out_cols = ['Isoform','Gene','Effective length','TPM'] + sr_per_sample_cols + lr_per_sample_cols
    all_SR_TPM_df[out_cols].sort_values(by=['Gene','Isoform']).to_csv(f'{output_path}/Isoform_abundance.out',sep='\t',index=False)
...
def EM_algo_hybrid_multi(isoform_len_dict,isoform_gene_dict,gene_isoforms_dict,sr_sam_list,lr_align_dirs,output_path,threads,EM_choice,lr_weights=None,sr_weights=None,use_quality_weights=False,alpha='adaptive'):
    ...
    for i,sr_sam in enumerate(sr_sam_list):
        ...
        if sr_weights is None and use_quality_weights:
    ...
    for i,lr_align_dir in enumerate(lr_align_dirs):
        ...
        if lr_weights is None and use_quality_weights:
    ...
    # SR-only多样本模式：没有LR数据，需创建空cond_prob占位文件供construct_community_helper使用
    if not cond_prob_dirs:
        canonical_cond_dir = f'{output_path}/temp/cond_prob'
        Path(canonical_cond_dir).mkdir(exist_ok=True, parents=True)
        n_isoforms = len(isoform_len_dict)
        for worker_id in range(threads):
            empty_cp = scipy.sparse.csr_matrix((0, n_isoforms))
            scipy.sparse.save_npz(f'{canonical_cond_dir}/{worker_id}_cond_prob.npz', empty_cp)
    ...
    if sr_sam_list:
        ...
        if use_quality_weights:
            print(f'[INFO] Weight mode: unique mapping rate. Global weights (all sum=1): LR={[round(w,4) for w in config.lr_global_weights]}, SR={[round(w,4) for w in config.sr_global_weights]}')
        elif lr_weights is not None or sr_weights is not None:
            print(f'[INFO] Weight mode: user-specified. Global weights (all sum=1): LR={[round(w,4) for w in config.lr_global_weights]}, SR={[round(w,4) for w in config.sr_global_weights]}')
        else:
            print(f'[INFO] Weight mode: equal weights. Global weights (all sum=1): LR={[round(w,4) for w in config.lr_global_weights]}, SR={[round(w,4) for w in config.sr_global_weights]}')
        # alpha设置：SR-only多样本时强制alpha=0（纯SR），否则按用户参数设置
        if n_lr == 0:
            config.alpha = 0.0
            config.alpha_df_path = None
            config.lr_global_weights = []
        elif alpha == 'adaptive':
