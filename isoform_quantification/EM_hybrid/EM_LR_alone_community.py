...
def EM_manager(isoform_gene_dict,isoform_index_dict,eff_len_arr,output_df,output_path,threads,n_reads_per_sample=None):
    ...
    # 按样本输出分列 num_expected_LRs_k，供 identifiability 读取 N
    lr_per_sample_cols = []
    if n_reads_per_sample is not None:
        for k, n_k in enumerate(n_reads_per_sample):
            col = f'num_expected_LRs_{k+1}'
            all_LR_TPM_df[col] = all_LR_TPM_df['theta'] * n_k
            lr_per_sample_cols.append(col)
    else:
        lr_per_sample_cols = ['num_expected_LRs']
    out_cols = ['Isoform','Gene','Effective length','TPM'] + lr_per_sample_cols
    all_LR_TPM_df[out_cols].sort_values(by=['Gene','Isoform']).to_csv(f'{output_path}/Isoform_abundance.out',sep='\t',index=False)
    all_iteration_df = pd.concat(all_iteration_df)
    all_iteration_df.to_csv(f'{output_path}/EM_iterations.tsv',sep='\t',index=False)
    duration = (time.time() - st)
    print(f'[INFO] Done in {duration} seconds!')
def EM_algo_LR_alone_multi(isoform_len_dict,isoform_gene_dict,lr_align_dirs,output_path,threads,EM_choice):
    """多LR数据集等权重合并后仅LR量化。"""
    ...
    # 计算每个 LR 样本的总读段数，用于 Isoform_abundance.out 分列输出
    n_reads_per_sample = []
    for cond_prob_dir in cond_prob_dirs:
        n_k = sum(
            scipy.sparse.load_npz(f'{cond_prob_dir}/{worker_id}_cond_prob.npz').shape[0]
            for worker_id in range(threads)
            if Path(f'{cond_prob_dir}/{worker_id}_cond_prob.npz').exists()
        )
        n_reads_per_sample.append(n_k)
    # 合并（等权重w_p=1，等价于vstack）
    ...
    EM_manager(isoform_gene_dict,isoform_index_dict,eff_len_arr,output_df,output_path,threads,n_reads_per_sample=n_reads_per_sample)
