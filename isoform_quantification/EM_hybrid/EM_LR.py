...
def prepare_LR(isoform_len_df,isoform_index_dict,isoform_index_series,threads,output_path,lr_align_dir=None,cond_prob_dir=None):
    if lr_align_dir is None:
        lr_align_dir = f'{output_path}/temp/LR_alignments'
    if cond_prob_dir is None:
        cond_prob_dir = f'{output_path}/temp/cond_prob'
    expression_dict = {}
    read_len_dist_dict = {}
    for fpath in glob.glob(f'{lr_align_dir}/dist_*_*'):
    ...
    sm_dict_save_path = f'{output_path}/read_len_dist_sm_dict'
    with open(sm_dict_save_path,'wb') as f:
    ...
    num_batches_dict = get_cond_prob_MT_LIQA_modified(threads,output_path,isoform_df,isoform_index_dict,read_len_dist,Sm_dict,lr_align_dir=lr_align_dir,cond_prob_dir=cond_prob_dir)
...
def merge_cond_prob_files(cond_prob_dirs,output_path,threads):
    """将多个LR数据集的cond_prob文件合并到规范目录 temp/cond_prob/。"""
    import scipy.sparse
    canonical_dir = f'{output_path}/temp/cond_prob'
    Path(canonical_dir).mkdir(exist_ok=True,parents=True)
    for worker_id in range(threads):
        matrices = []
        for d in cond_prob_dirs:
            fpath = f'{d}/{worker_id}_cond_prob.npz'
            if Path(fpath).exists():
                matrices.append(scipy.sparse.load_npz(fpath))
        if matrices:
            merged = scipy.sparse.vstack(matrices)
            scipy.sparse.save_npz(f'{canonical_dir}/{worker_id}_cond_prob.npz',merged)
