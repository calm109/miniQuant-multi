...
def get_all_reads_isoform_cond_prob_LIQA_modified(args):
    worker_id,output_path,isoform_df,isoform_index_dict,read_len_dist,Sm_dict = args[:6]
    lr_align_dir = args[6] if len(args) > 6 else None
    cond_prob_dir = args[7] if len(args) > 7 else None
    ...
    for fpath in sorted(glob.glob(f'{lr_align_dir}/reads_{worker_id}_*')):
        ...
        num_batches += 1
    if len(all_cond_prob_matrix) == 0:
        cond_prob_matrix = scipy.sparse.csr_matrix((0, len(isoform_index_dict)))
    else:
        cond_prob_matrix = scipy.sparse.vstack(all_cond_prob_matrix)
    Path(cond_prob_dir).mkdir(exist_ok=True,parents=True)
    scipy.sparse.save_npz(f'{cond_prob_dir}/{worker_id}_cond_prob.npz',cond_prob_matrix)
...
def get_cond_prob_MT_LIQA_modified(threads,output_path,isoform_df,isoform_index_dict,read_len_dist,Sm_dict,lr_align_dir=None,cond_prob_dir=None):
    pool = mp.Pool(threads)
    futures = []
    for worker_id in range(threads):
        args = worker_id,output_path,isoform_df,isoform_index_dict,read_len_dist,Sm_dict,lr_align_dir,cond_prob_dir
