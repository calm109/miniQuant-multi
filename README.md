...
<object data="miniQuant-multi.pdf" type="application/pdf" width="100%" height="500px">
    <p>你的浏览器不支持直接预览 PDF，请点击 <a href="miniQuant-multi.pdf">下载 PDF 文件</a> 查看。</p>
</object>
---
...
bash run_miniquant_multi_example.sh
```
---
...
| `kvalues_isoform.out` | Per-isoform attributes: isoform length, exon count, and gene membership. Join with `kvalues_gene.out` on `Gene` for A-matrix condition number metrics |
| `identifiability.tsv` | Per-gene identifiability metrics (requires EM quantification results): minimum positive singular value of the restricted Jacobian (`_rJacobi_sigma_min_pos`), restricted Jacobian k-value (`_rJacobi_k_value`), restricted Fisher information matrix metrics including minimum eigenvalue (`_rfisher_lambda_min`), log10 eigenvalue condition number (`_rfisher_log10_lambda_cond`), log10 determinant (`_rfisher_det` = log10(\|det(F_tan)\|+1)), and identifiability flag (`_rfisher_identifiable`), for each sample and platform (LR, SR, Hybrid) |
| `isoform_SE_CI.tsv` | Per-isoform standard errors and 95% confidence intervals of EM-estimated abundance (θ), computed from the restricted Fisher information matrix |
